...
class OrderItemsPage {
    public static function render_page() {
        ...
        ?>
        <div class="wrap">
            <h1><?php esc_html_e('Order Items Status', 'woocommerce-production-manager'); ?></h1>
            <form method="get">
                <input type="hidden" name="page" value="wpm-order-items">
                <div class="wpm-filters">
                    <input type="text" name="order_id" placeholder="<?php esc_attr_e('Order ID', 'woocommerce-production-manager'); ?>" value="<?php echo esc_attr($_GET['order_id'] ?? ''); ?>">
                    <input type="text" name="customer_name" placeholder="<?php esc_attr_e('Customer Name', 'woocommerce-production-manager'); ?>" value="<?php echo esc_attr($_GET['customer_name'] ?? ''); ?>">
                    <select name="status">
                        <option value=""><?php esc_html_e('All Statuses', 'woocommerce-production-manager'); ?></option>
                    ...
                    <button type="button" class="button wpm-export-order-items-csv"><?php esc_html_e('Export to CSV', 'woocommerce-production-manager'); ?></button>
                    <button type="button" class="button wpm-export-order-items-excel"><?php esc_html_e('Export to Excel', 'woocommerce-production-manager'); ?></button>
                </div>
            </form>
            <form method="post">
                <?php $table->display(); ?>
    ...
    public static function export_order_items_csv() {
        ...
        fputcsv($file, [
            __('Order ID', 'woocommerce-production-manager'),
            __('Item Name', 'woocommerce-production-manager'),
            __('Customer', 'woocommerce-production-manager'),
            __('Order Date', 'woocommerce-production-manager'),
            __('Status', 'woocommerce-production-manager'),
            __('Delivery Date', 'woocommerce-production-manager')
        ]);
        ...
        foreach ($items as $item) {
            fputcsv($file, [
                ...
                $item->customer_name ?: __('Guest', 'woocommerce-production-manager'),
    ...
    public static function export_order_items_excel() {
        ...
        foreach ($items as $item) {
            ...
            $sheet->setCellValue("C$row", $item->customer_name ?: __('Guest', 'woocommerce-production-manager'));
